...
Files to create:
- `requirements.txt` — see README.md for full list
- `.env.example` — see README.md for all variables
...
Files to create/fill:
- `backend/db/models.py` — all 4 SQLModel tables (see README.md)
...
In `prompts.py`: Define all prompt templates as Python string constants. See README.md for exact prompts:
- `TRIAGE_PROMPT`
- `PERSONA_PROMPT`  
- `REVIEW_COMMENT_PROMPT`
- `CLUSTER_PROMPT`

In `mock_responses.py`: Define realistic mock returns for each prompt type. See README.md for mock data.
...
4. Apply formula from README.md
...
2. Compute base_risk from formula in README.md
3. Map to risk_level
